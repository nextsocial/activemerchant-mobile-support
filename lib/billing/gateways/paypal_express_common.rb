module ActiveMerchant
  module Billing
    module PaypalExpressCommon
      ...
      def mobile_redirect_url(token, options = {})
        options = {
            :review => true,
            :mobile => true
        }.update(options)

        url  = "#{redirect_url}?cmd=_express-checkout-mobile&token=#{token}"
        url += '&useraction=commit' unless options[:review]
        url
      end

      def desktop_redirect_url(token, options = {})
        options = {
            :review => true,
            :mobile => false
        }.update(options)

        url  = "#{redirect_url}?cmd=_express-checkout&token=#{token}"
        url += '&useraction=commit' unless options[:review]
        url
      end
