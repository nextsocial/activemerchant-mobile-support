  ...
  module Billing #:nodoc:
    ...
    module PaypalCommonAPI
      ...
      def commit(action, request)
        ...
        build_commit_response(response)
      end
      ...
      def commit_mobile_sale(params)
        ...
        request = {
            ...
            :PAYMENTREQUEST_0_AMT          => to_decimal(params[:money]),
        ...
        if response.present?
          ...
          response.keys.each{|key| response[key.downcase.to_sym] = response[key] ; response.delete(key)}
        end
        build_commit_response(response)
      end
      ...
      def endpoint_mobile_url
        URLS_MOBILE[test? ? :test : :live][@options[:signature].blank? ? :certificate : :signature]
      end

      def build_commit_response(response)
        build_response(
            successful?(response),
